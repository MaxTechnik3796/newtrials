	...
	public boolean hurtEnemy(@NotNull ItemStack stack,@NotNull LivingEntity target,@NotNull LivingEntity attacker){
		if(attacker instanceof Player player){
			...
			if(fallDistance>MIN_FALL_DISTANCE){
				// Smash attack effects (sounds and knockback)
				if(fallDistance>15.0F){
					...
					player.level().playSound(null,player.getX(),player.getY(),player.getZ(),
							NTrialsModSounds.MACE_SMASH_AIR.get(),SoundSource.PLAYERS,1.0F,0.8F+Math.min(fallDistance/30.0F,0.2F));
				}
				if(fallDistance>3.0F){
					float kb=Math.min(fallDistance/10.0F,2.0F);
				...
				// Smash attack negates fall damage
