...
public class BreezeBossEntity extends Monster {
    ...
    public BreezeBossEntity(EntityType<? extends Monster> entityType, Level level) {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, 60.0D) // 2x HP (30 * 2 = 60)
                .add(Attributes.ATTACK_DAMAGE, 4.0D) //set attack damage
                .add(Attributes.MOVEMENT_SPEED, 0.6D) //set movement speed
    ...
    public void tick() {
        ...
        // Melee attack check - like blaze
        if (!this.level().isClientSide) {
            LivingEntity target = this.getTarget();
            if (target != null) {
                double distance = this.distanceToSqr(target);
                if (distance < 4.0D && this.hasLineOfSight(target) && meleeAttackCooldown <= 0) {
    ...
    public boolean doHurtTarget(net.minecraft.world.entity.Entity target) {
        if (target instanceof LivingEntity livingTarget) {
            ...
            float damage = 12.0F;
            boolean hurt = livingTarget.hurt(this.damageSources().mobAttack(this), damage);
            if (hurt) {
    ...
    private void updateFloating() {
        ...
        if (target != null) {
            ...
            double targetY = target.getY() + this.allowedHeightOffset;
    ...
    static class BreezeBossAttackGoal extends Goal {
        private final BreezeBossEntity breeze;
        private int attackTime;

        public BreezeBossAttackGoal(BreezeBossEntity breeze) {
            this.breeze = breeze;
        }
    ...
    static class BreezeBossKeepDistanceGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            // This goal is now handled by BreezeBossChaseGoal
            return false;
        }
    ...
    static class BreezeBossCombatJumpGoal extends Goal {
        private final BreezeBossEntity breeze;
        ...
        public BreezeBossCombatJumpGoal(BreezeBossEntity breeze) {
            this.breeze = breeze;
