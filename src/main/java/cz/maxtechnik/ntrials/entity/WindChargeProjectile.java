...
public class WindChargeProjectile extends ThrowableItemProjectile {
	...
	private void createWindExplosion() {
		...
		if (!this.level().isClientSide) {
				...
				if (entity instanceof LivingEntity living) {
					...
					if (distance <= radius) {
						...
						if (this.getOwner() instanceof BreezeEntity) {
							...
							living.hurt(this.damageSources().explosion(this, this.getOwner()), damage);
						} else if (this.getOwner() instanceof cz.maxtechnik.ntrials.entity.BreezeBossEntity) {
							float damage = 9.0F; // 1.5x base damage
							living.hurt(this.damageSources().explosion(this, this.getOwner()), damage);
						}

						Vec3 direction = living.position().subtract(explosion_center).normalize();
			...
			// ==================== NOVÁ FUNKCE: Imunita proti fall damage po odpálení ====================
			// Uložíme čas a pozici exploze pro každého hráče, který byl v dosahu (byl odpálen)
			for (Entity entity : entities) {
				if (entity instanceof Player player) {
					double distance = player.distanceTo(this);
					if (distance <= radius) { // Byl skutečně zasažen explozi
						CompoundTag data = player.getPersistentData();

						long currentTime = this.level().getGameTime();

						// Uložíme čas exploze a pozici (pro pozdější kontrolu v LivingFallEvent)
						data.putLong("WindChargeImmunityTime", currentTime);
						data.putDouble("WindChargeExplosionX", explosion_center.x);
						data.putDouble("WindChargeExplosionY", explosion_center.y);
						data.putDouble("WindChargeExplosionZ", explosion_center.z);
					}
				}
			}
			// =========================================================================================
				...
				for (double offsetZ = -2.0D; offsetZ <= 2.0D; offsetZ += 1.0D) {
					...
					try {
						if (block == Blocks.REDSTONE_WIRE || block instanceof RedStoneWireBlock) {
							...
							try {
								BlockState newState = blockState.setValue(RedStoneWireBlock.POWER,15);
								this.level().setBlock(blockPos, newState, 3);
